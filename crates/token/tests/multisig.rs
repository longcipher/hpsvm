...
fn test() {
    ...
    let mint_pk = CreateMint::new(svm, &payer_kp).authority(&multisig_pk).send().unwrap();
    ...
    let multisig_account_pk =
        CreateAccount::new(svm, &payer_kp, &mint_pk).owner(&multisig_pk).send().unwrap();
    ...
    let random_account_pk =
        CreateAccount::new(svm, &payer_kp, &mint_pk).owner(&random_pk).send().unwrap();
    ...
    SetAuthority::new(svm, &payer_kp, &random_account_pk, AuthorityType::CloseAccount)
        .owner(&random_kp)
