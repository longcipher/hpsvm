...
fn test_set_compute_budget() {
    ...
    let tx_fee = 5000;
    svm.airdrop(&from, svm.get_sysvar::<Rent>().minimum_balance(0) + tx_fee + 100).unwrap();
...
fn test_priority_fee_is_charged() {
    ...
    assert_eq!(
        final_balance,
        initial_balance - total_fee - transfer_amount,
    ...
    assert_eq!(recipient_balance, initial_recipient_balance + transfer_amount);
}
