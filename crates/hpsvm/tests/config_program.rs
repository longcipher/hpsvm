...
fn deserialize_account_data<T: DeserializeOwned>(bytes: &[u8]) -> T {
    Account { data: bytes.to_vec(), ..Default::default() }.deserialize_data().unwrap()
}

fn try_deserialize_account_data<T: DeserializeOwned>(bytes: &[u8]) -> Option<T> {
    Account { data: bytes.to_vec(), ..Default::default() }.deserialize_data().ok()
}

fn serialized_len<T: Serialize>(value: &T) -> usize {
    Account::new_data(0, value, &solana_config_interface::id()).unwrap().data().len()
}
...
fn get_config_space(key_len: usize) -> usize {
    let entry_size = serialized_len(&(Address::default(), true));
    serialized_len(&(ConfigKeys::default(), MyConfig::default())) + key_len * entry_size
}
...
fn create_config_account(
    ...
    let max_space = serialized_len(&MyConfig::default()) as u64;
...
fn test_process_create_ok() {
    ...
    assert_eq!(
        ...
        MyConfig::deserialize(get_config_data(config_account.data()).unwrap())
    );
}
...
fn test_process_store_with_additional_signers() {
    ...
    let config_account = context.svm.get_account(&config_keypair.pubkey()).unwrap();
    let config_state: ConfigKeys = deserialize_account_data(config_account.data());
...
fn test_process_store_bad_config_account() {
    ...
    context
        .svm
        .set_account(signer0.pubkey(), Account::new(100_000, 0, &solana_config_interface::id()))
        .unwrap();
...
fn test_config_bad_owner() {
    ...
    context
        .svm
        .set_account(config_keypair.pubkey(), Account::new(lamports, 0, &Address::new_unique()))
        .unwrap();
