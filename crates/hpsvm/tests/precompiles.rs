...
fn ed25519_precompile_err() {
    ...
    assert_eq!(
        ...
        Some(TransactionError::InstructionError(0, InstructionError::Custom(2)))
    );
}
...
fn secp256k1_precompile_ok() {
    ...
    // k256's to_sec1_point(false) also produces uncompressed format (0x04 || X || Y).
    let pub_bytes = secp_pubkey.to_sec1_point(false);
