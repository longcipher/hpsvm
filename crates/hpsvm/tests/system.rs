...
fn system_transfer() {
    ...
    assert!(tx_res.is_ok());
    assert_eq!(from_account.unwrap().lamports, original_balance - expected_fee - transfer_amount);
    assert_eq!(to_account.unwrap().lamports, original_balance + transfer_amount);
}
