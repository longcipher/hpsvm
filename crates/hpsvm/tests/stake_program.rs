...
use hpsvm::HPSVM;
use solana_account::{Account, ReadableAccount, WritableAccount, state_traits::StateMut};
    ...
    for _ in 0..number_of_credits {
        ...
        if vote_state.epoch_credits.is_empty() {
            ...
            if credits_val != prev_credits {
                ...
                vote_state.epoch_credits.push((epoch, credits_val, credits_val));
            } else {
        ...
        vote_state.epoch_credits.last_mut().unwrap().1 =
            vote_state.epoch_credits.last().unwrap().1.saturating_add(credits);
    }
...
impl Accounts {
    fn initialize(&self, svm: &mut HPSVM, payer: &Keypair) {
...
fn advance_epoch(svm: &mut HPSVM) {
...
fn get_account(svm: &mut HPSVM, pubkey: &Address) -> Account {
    svm.get_account(pubkey).expect("account not found")
}

fn get_stake_account(svm: &mut HPSVM, pubkey: &Address) -> (Meta, Option<Stake>, u64) {
...
fn get_stake_account_rent(svm: &mut HPSVM) -> u64 {
    let rent = svm.get_sysvar::<Rent>();
    rent.minimum_balance(std::mem::size_of::<stake::state::StakeStateV2>())
}

fn get_minimum_delegation(svm: &mut HPSVM, payer: &Keypair) -> u64 {
    let transaction = Transaction::new_signed_with_payer(
    ...
    let mut data = svm.simulate_transaction(transaction).unwrap().meta.return_data.data;
    data.resize(8, 0);
...
fn create_blank_stake_account(svm: &mut HPSVM, payer: &Keypair) -> Address {
    let stake = Keypair::new();
...
fn test_stake_checked_instructions() {
    ...
    let stake =
        create_independent_stake_account(&mut svm, &Authorized { staker, withdrawer }, 0, &payer);
    let instruction =
        ixn::authorize_checked(&stake, &withdrawer, &authorized, StakeAuthorize::Withdrawer, None);
...
fn test_stake_initialize() {
    ...
    let lockup = Lockup { epoch: 1, unix_timestamp: 0, custodian };
...
fn test_authorize() {
    ...
    for (authority, authority_type) in
        [(&stakers[0], StakeAuthorize::Staker), (&withdrawers[0], StakeAuthorize::Withdrawer)]
    {
    ...
    let authorized =
        Authorized { staker: stakers[0].pubkey(), withdrawer: withdrawers[0].pubkey() };
    ...
    for (old_authority, new_authority, authority_type) in [
        ...
        (&withdrawers[0], Address::new_unique(), StakeAuthorize::Withdrawer),
    ] {
        let instruction =
            ixn::authorize(&stake, &old_authority.pubkey(), &new_authority, authority_type, None);
        let e =
    ...
    for staker in stakers {
        ...
        let instruction =
            ixn::withdraw(&stake, &staker.pubkey(), &recipient, rent_exempt_reserve, None);
...
fn test_stake_delegate() {
    ...
    svm.set_account(fake_vote_address, fake_vote_account).unwrap();
    ...
    rewards_pool.set_state(&StakeStateV2::RewardsPool).unwrap();
    svm.set_account(rewards_pool_address, rewards_pool).unwrap();

    let instruction =
        ixn::delegate_stake(&rewards_pool_address, &staker, &accounts.vote_account.pubkey());
