...
- Restore `Send` and `Sync` traits on `HPSVM` struct ([#266](https://github.com/LiteSVM/litesvm/pull/266)).
...
- Make the HPSVM struct thread-safe ([#127](https://github.com/LiteSVM/litesvm/pull/127)).
...
- Add `HPSVM::with_precompiles` ([#102](https://github.com/LiteSVM/litesvm/pull/102)).
...
- Add `error.rs` and new `HPSVMError` type ([#62](https://github.com/LiteSVM/litesvm/pull/62)).
...
- Change `set_account` to return `Result<(), HPSVMError>` ([#62](https://github.com/LiteSVM/litesvm/pull/62)).
